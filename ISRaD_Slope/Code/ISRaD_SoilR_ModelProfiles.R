...
OPM_VTLM_k <- 
  purrr::map(k1_list, \(x) OnepModel14(
    t = years, 
    k = x, 
    C0 = initialC,
...
## Step 2. Create SoilR pool model & Create Vertical Transport model
OPM_VTLM_LI <- 
  purrr::map(LI_list, \(x) OnepModel14(
    t = years, 
    k = k1, 
    C0 = initialC,
    F0_Delta14C = initialF14, 
...
OPM_VTLM_a <- 
  purrr::map(a21_list, \(x) TwopSeriesModel14(
    t = years, 
    ks = c(k1, k2), 
    C0 = initialC,
    a21 = x,
...
L14t_a <- purrr::map2(Cdt_a, C14dt_a, 
...
ggsave(file = paste0("./Figure/Figure_6_", Sys.Date(), ".png"),
       width = 11, height = 14)


#### Different model set-ups

### Figure A4: One-pool model with different k-values that are constant with depth
## Step 1. Prepare what you need for a SoilR pool model
# Parameters for one-pool model
years <- seq(-48050,2019, by = 1)
# years <- seq(1941.5,2019, by = 0.5)
...
OPM_results_k %>% 
  ...
  theme(panel.background = element_blank(),
        ...
        legend.background = element_blank()) +
  scale_x_continuous("Soil organic carbon stocks; log-scale", 
                     limits = c(1e-14,1000), trans = "log10",
                     breaks = c(1e-14,1e-12,1e-10,1e-8,1e-6,1e-4,1e-2,1e-0,1,100), 
                     expand = c(0,0),
                     labels = c(1e-14,1e-12,1e-10,1e-8,1e-6,1e-4,1e-2,1e-0,1,100)) +
  scale_y_continuous(expression(paste(Delta^14,"C [‰]")), 
                     limits = c(-300,300), expand = c(0,0), breaks = seq(-300,300,100)) +
  scale_shape_manual("depth layer", values = c(25, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21)) +
  scale_linewidth_manual("", values = c(2,2,2,2,2,3,2,2)) +
  scale_color_manual("k-value",
  ...
  guides(shape = "none", linewidth = "none", 
         color = guide_legend(override.aes = list(linewidth = 2)))
ggsave(file = paste0("./Figure/Figure_A4_", Sys.Date(), ".png"),
       width = 12, height = 6)

### Figure A5: One-pool model with different vertical transport rates
## Step 1. Prepare what you need for a SoilR pool model
# Parameters for one-pool model
k1 <- 1/500
LitterInput <- 1
...
OPM_results_d %>% 
  ...
  theme(panel.background = element_blank(),
        ...
        legend.background = element_blank()) +
  scale_x_continuous("Soil organic carbon stocks; log-scale", 
                     limits = c(1e-10,1000), trans = "log10",
                     breaks = c(1e-10,1e-8,1e-6,1e-4,1e-2,1e-0,1,100), 
                     expand = c(0,0),
                     labels = c(1e-10,1e-8,1e-6,1e-4,1e-2,1e-0,1,100)) +
  scale_y_continuous(expression(paste(Delta^14,"C [‰]")), 
                     limits = c(-500,250), expand = c(0,0)) +
  scale_shape_manual("depth layer", values = c(25, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21)) +
  scale_linewidth_manual("", values = c(2,2,2,3,2,2,2,2,2)) +
  scale_color_manual("vertical transport",
                     values = c("#c7eae5", "#5ab4ac", "#01665e",
                                "#7b3294",
...
ggsave(file = paste0("./Figure/Figure_A5_", Sys.Date(), ".png"),
       width = 12, height = 6)
