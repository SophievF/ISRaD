...
mspline_14c_c_all %>% 
  group_by(ClimateZoneAnd) %>% 
  mutate(pro_AI_mod = 0.3 * pro_AI + pro_AI) %>% 
  # mutate(pro_AI_mod = pro_AI - (0.3 * pro_AI)) %>% 
  # mutate(pro_AI_mod = 1 + pro_AI) %>% 
  skimr::skim(pro_AI, pro_AI_mod)

rf_data_14c_AI <- mspline_14c_c_all %>% 
  # mutate(pro_AI = pro_AI - (0.3 * pro_AI)) %>% 
  # mutate(pro_AI = 1 + pro_AI) %>% 
...
rf_data_pred_14c_c_AI <- cbind(mspline_14c_c_all, rf_pred_14c_AI, rf_pred_c_AI) %>% 
  tibble()
...
c1_14c_c_AI <- climate_all_14c_c_AI %>% 
  ...
  scale_y_continuous(expression(paste("Predicted ", Delta^14, "C [‰]")), expand = c(0,0),
                     limits = c(-1000,125), breaks = seq(-1000,0,250)) 
plot(c1_14c_c_AI)
ggsave(file = paste0("./Figure/ISRaD_msp_pred14C_pred_SOC_climate_AImod_pos30_", Sys.Date(),
                     ".jpeg"), width = 6, height = 5.5)
...
rf_data_14c_GPP <- mspline_14c_c_all %>% 
  ...
  mutate(pro_GPP_Fluxcom_2001_2012_gC_m2d1 = 0.202) %>%  
  dplyr::select(id, entry_name, UD, lyr_14c_msp,
                pro_MAT_mod, pro_GPP_Fluxcom_2001_2012_gC_m2d1,
                lyr_clay_mod, pro_AI) 
...
rf_data_pred_14c_c_GPP <- cbind(mspline_14c_c_all, rf_pred_14c_GPP, rf_pred_c_GPP) %>% 
  tibble()
