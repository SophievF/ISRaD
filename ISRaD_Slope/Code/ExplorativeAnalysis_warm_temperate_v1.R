...
lyr_data <- read_csv("./Data/ISRaD_flat_splined_filled_2023-02-06.csv")
...
plot(wtemp_soil)
ggsave(file = paste0("./Figure/ISRaD_msp_14C_SOC_climate_wtemp_soil_", Sys.Date(),
                     ".jpeg"), width = 5, height = 5)

wtemp_mineral <- wtemp_msp %>% 
  mutate(MineralType_new = case_when(
    pro_usda_soil_order == "Entisols" ~ "entisols",
    pro_usda_soil_order == "Alfisols" ~ "alfisols",
    TRUE ~ MineralType
  )) %>% 
...
plot(wtemp_mineral)
ggsave(file = paste0("./Figure/ISRaD_msp_14C_SOC_climate_wtemp_mineral_", Sys.Date(),
                     ".jpeg"), width = 5, height = 5)

wtemp_climate <- wtemp_msp %>% 
...
plot(wtemp_climate)
ggsave(file = paste0("./Figure/ISRaD_msp_14C_SOC_climate_wtempclimate_", Sys.Date(),
                     ".jpeg"), width = 5, height = 5)
...
plot(wtemp_clay_grp)
ggsave(file = paste0("./Figure/ISRaD_msp_14C_SOC_climate_wtemp_clay_30_", Sys.Date(),
                     ".jpeg"), width = 5, height = 5)
