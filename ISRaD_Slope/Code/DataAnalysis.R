...
c1_14c_c <- climate_14c_c %>% 
  ...
  geom_errorbarh(aes(xmin = lci_c, xmax = uci_c, y = median_14c, 
                     color = ClimateZoneAnd), alpha = 0.3) +
  geom_point(data = depth_sum_c, aes(x = median_c, y = median_14c), size = 2,
...
ggarrange(annotate_figure(ggarrange(c1_14c, c1_c, nrow = 2, legend = "none"),
          ...
          c1_14c_c, common.legend = TRUE, widths = c(0.8,1))
ggsave(file = paste0("./Figure/ISRaD_msp_SOC_14C_climate_depth_", Sys.Date(),
...
m1_14c_c <- mineral_14c_c %>% 
  ...
  geom_errorbarh(aes(xmin = lci_c, xmax = uci_c, y = median_14c, 
                     color = MineralType), alpha = 0.3) +
  geom_point(data = depth_sum_m, aes(x = median_c, y = median_14c), size = 2,
             shape = 21, fill = "black", color = "white") +
  theme_bw(base_size = 16) +
  theme(axis.text = element_text(color = "black"),
        legend.position = c(0.21,0.2),
...
ggarrange(annotate_figure(ggarrange(m1_14c, m1_c, nrow = 2, legend = "none"),
                          left = text_grob("Depth [cm]", rot = 90, size = 16)), 
          m1_14c_c, common.legend = TRUE, widths = c(0.8,1))
...
p_rf_c <- rr_c_prediction %>% 
  ...
  theme_bw(base_size = 16) +
  theme(axis.text = element_text(color = "black")) +
  scale_y_continuous("Observed SOC [wt-%]", limits = c(0,20)) +
...
vi_14c <- vi_14c_sum %>% 
  ggplot() +
  geom_bar(aes(x = reorder(predictor, -median), y = median),
           stat = "identity") +
...
mat_ice %>% 
  ggplot(aes(y = pred_median_14c, x = pred_median_c, color = ClimateZoneAnd)) +
  geom_point(aes(size = pro_MAT_mod)) +
  theme_bw() +
  theme_bw(base_size = 14) +
  theme(axis.text = element_text(color = "black")) +
  scale_y_continuous(expression(paste("Median predicted ", Delta^14,"C [‰]")), 
                     limits = c(-520,0), expand = c(0,0)) +
  scale_x_continuous("Mean predicted SOC [wt-%]; log-scaled", expand = c(0,0), 
                     breaks = c(0.1,0.5,1,5,10),
                     labels = c(0.1,0.5,1,5,10)) +
  coord_trans(x = "log10", xlim = c(0.1,10)) +
  annotation_logticks(sides = "b", scaled = FALSE,
                      short = unit(1.5,"mm"),
                      mid = unit(3,"mm"),
                      long = unit(4,"mm")) +
...
ggsave(file = paste0("./Figure/ISRaD_msp_SOC_14c_AndiPolar_ice_climate_mat_", 
                     Sys.Date(), ".jpeg"), width = 12, height = 6)
...
ggsave(file = paste0("./Figure/ISRaD_msp_SOC_14c_AndiPolar_ice_climate_ai_", 
                     Sys.Date(), ".jpeg"), width = 12, height = 6)
...
lyr_data %>% 
  ...
  filter(ClimateZoneAnd != "tundra/polar") %>%
  group_by(ClimateZoneAnd) %>% 
  dplyr::summarise(tibble::enframe(quantile(lyr_clay_mod, c(0.05,0.95)), 
                                   "quantile", "lyr_clay_mod")) %>% 
  ungroup()
...
ggsave(file = paste0("./Figure/ISRaD_msp_SOC_14c_AndiPolar_ice_climate_clay_", 
                     Sys.Date(), ".jpeg"), width = 12, height = 6)
...
clay_14c_c_p <- clay_p %>% 
  ...
  geom_segment(x = 0.343, y = -91.1, xend = 0.7, yend = -200, 
               color = color_climate_wo_polar[4],
               arrow = arrow(length = unit(0.5, "cm")), linetype = "dashed") +
  geom_segment(x = 0.717, y = -70.4, xend = 0.78, yend = -125,
               color = color_climate_wo_polar[3],
               arrow = arrow(length = unit(0.5, "cm")), linetype = "dashed") +
  geom_segment(x = 0.840, y = -115, xend = 1.3, yend = -205,
               color = color_climate_wo_polar[2],
               arrow = arrow(length = unit(0.5, "cm")), linetype = "dashed") +
  geom_segment(x = 1.06, y = -123, xend = 1.53, yend = -243,
